...
{
    // Total number of particles of the cloth Nu x Nv
    const size_t N = force.size();
     // Number of particles along one dimension (square dimension)
    ...
    for(size_t k = 0; k < N; ++k)
        force[k] = m * g; // force gravity = mass x acceleration
        ...
        for(int kv = 0; kv < N_dim; ++kv) 
        {
            ...
            if (ku > 0)
            {
                ...
                if (kv > 0)
                {
                    ...
                    vec3 normal_tmp = normalize(
                        cross(
                            normalize(position[up] - position[current]),
            ...
            if (ku < N_dim - 1)
            {
                ...
                if (kv > 0)
                {
                    ...
                    vec3 normal_tmp = normalize(
                        cross(
                            normalize(position[down] - position[current]),
                            normalize(position[left] - position[current])
                        )
                    );
            ...
            for (int i = -2; i < 3; i++)
            {
                for (int j = -2; j < 3; j++)
                {
                    ...
                    if (ku + i >= 0 && ku + i < N_dim && kv + j >= 0 && kv + j < N_dim)
                    {
                        ...
                        // L0 for structural springs
                        float Lij = L0;
                        
                         // L0 for shearing springs
                        if (i != 0 && j != 0)
                            Lij = sqrt(2 * L0 * L0);

                        // L0 for bending springs
                        if (abs(i) == 2 || abs(j) == 2)
                            Lij = 2 * L0;
...
void scene_model::collision_constraints()
{
    ...
    for (size_t k = 0; k < N; ++k)
    {
        ...
        if (detection <= collision_shapes.sphere_r + epsilon)
        {
            ...
            float d = collision_shapes.sphere_r + epsilon - norm(position[k] - collision_shapes.sphere_p);
...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& gui)
{
    ...
    if( (!simulation_diverged || force_simulation) && h>0)
    {
        ...
        for(size_t k=0; (!simulation_diverged  || force_simulation) && k<number_of_substeps; ++k)
        {
            ...
            // Detect and solve collision with other shapes
            collision_constraints();

            // Enforce hard positional constraints
            hard_constraints();

            // Update normals of the cloth
            normal(position.data, connectivity, normals);
            // Check if the simulation seems to diverge
            detect_simulation_divergence();
        }
    }
